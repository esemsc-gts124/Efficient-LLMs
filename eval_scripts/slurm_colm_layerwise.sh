...
# Usage: sbatch eval_scripts/slurm_colm_layerwise.sh eval_scripts/colm-layerwise/<run_name>.sh
...
if [ -z "$RUN_SCRIPT" ]; then
    echo "Error: pass a run script, e.g.: sbatch eval_scripts/slurm_colm_layerwise.sh eval_scripts/colm-layerwise/250M_bulge.sh"
    exit 1
fi
