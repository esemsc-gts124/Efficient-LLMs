...
export OUTPUT_PATH="/checkpoint/optim/sanaelotfi/weight_shared_llms/eval_results/colm/attn_rank"
cd /storage/home/sanaelotfi/eval || { echo "Failed to cd to repo root"; exit 1; }
RUN_SCRIPT=$1
if [ -z "$RUN_SCRIPT" ]; then echo "Error: pass a run script"; exit 1; fi
if [ ! -f "$RUN_SCRIPT" ]; then echo "Error: script not found: $RUN_SCRIPT"; exit 1; fi
bash "$RUN_SCRIPT"
