#!/bin/bash
#SBATCH --job-name=layerwise_eval
...
# Usage: sbatch slurm_layerwise.sh eval_scripts/layerwise/<run_name>.sh
...
export REPO_PATH="/path/to/Efficient-LLMs"            # layer-sharing-final branch checkout
export OUTPUT_PATH="/path/to/eval_results/layerwise"   # where results will be saved
...
if [ -z "$RUN_SCRIPT" ]; then
    echo "Error: pass a run script as argument, e.g.: sbatch slurm_layerwise.sh eval_scripts/layerwise/250M_base.sh"
    exit 1
fi
