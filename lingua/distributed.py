...
class InterProcessComm:
    ...
    def __init__(self, distributed_args: DistributedArgs):
        ...
        self.training_group = getattr(distributed_args, '_training_group', None)
        self.distill_group = getattr(distributed_args, '_distill_group', None)
    ...
    def scatter_tensors_from_distill(self, tensors_to_scatter=None):
        ...
        if self.is_training:
            ...
            for rank in range(get_world_size()):
                if rank in self.training_ranks:
                    # Get the corresponding tensor for this training rank
                    idx = self.training_ranks.index(rank)
                    scatter_list.append(tensors_to_scatter[idx])
                else:
...
def get_device_mesh(distributed_args: DistributedArgs):
    ...
    if is_training:
        ...
        assert (
            dp_replicate * dp_shard * tp_size == training_world_size
        ), f"dp_replicate * dp_shard * tp_size ({dp_replicate} * {dp_shard} * {tp_size}) != training_world_size ({training_world_size})"
    else:
        # For distillation rank, we don't need a device mesh
        return None
